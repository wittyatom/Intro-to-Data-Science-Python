...
def answer_two():
    count=max(df['Total']-df['Total.1'])
    ans= df[(df['Total']-df['Total.1']) == count].index.tolist()
    return ans[0]
answer_two()
...
def answer_three():
    fr = df[(df['Gold'] > 0) & (df['Gold.1'] > 0)]
    x = max(abs(fr['Gold']-fr['Gold.1'])/fr['Gold.2'])
    ans= fr[(abs(fr['Gold']-fr['Gold.1'])/fr['Gold.2']) == x].index.tolist()
    return ans[0]
answer_three()
